	...
	for (int32 x = 0; x < GridSizeX; x++)
	{
		for (int32 y = 0; y < GridSizeY; y++)
		{
			if (PreviewMesh)
			{
				...
				// Проверяем есть ли что-то на месте превью-объекта
...
TSubclassOf<ABonusParent> APlayingBoard::GetBonusClass()
{
	...
	{
		TotalWeigh += CurrentBonus.DropChance * 100.f;
	}
	int32 RandomWeigh = FMath::RandHelper(TotalWeigh);

	for (const auto& CurrentBonus : BonusTypeByChance)
	{
		if (RandomWeigh >= CurrentBonus.DropChance * 100.f)
		{
			RandomWeigh -= CurrentBonus.DropChance * 100.f;
		}
		else
		{
